...
<script>
        ...
        const LARAVEL_WEB_UPLOAD_URL = "/pelanggancaters/";
        ...
        async function getAddressFromCoordinates(lat, lon) {
            ...
            try {
                const response = await fetch(
                    `${NOMINATIM_REVERSE_GEOCODING_API_URL}?format=json&lat=${lat}&lon=${lon}&zoom=18&addressdetails=1`
                );
        ...
        function checkSubmitButtonStatus() {
            ...
            // Enable submit button only if both images are either newly captured or already exist in DB
        ...
        async function uploadImages() {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    submitMessageEl.innerText =
                        `Gagal menyimpan gambar dan data: ${result.message || 'Terjadi kesalahan'}`;
                    submitMessageEl.style.color = 'red';
        ...
        @if (!empty($pelanggan->gambar_kwh) && !empty($pelanggan->kwh_latitude) && !empty($pelanggan->kwh_longitude))
            document.addEventListener('DOMContentLoaded', () => {
                initializeLeafletMap('mapKWH', parseFloat({{ $pelanggan->kwh_latitude }}), parseFloat(
                    {{ $pelanggan->kwh_longitude }}), 'KWH Tersimpan');
        ...
        @if (!empty($pelanggan->gambar_rumah) && !empty($pelanggan->rumah_latitude) && !empty($pelanggan->rumah_longitude))
            document.addEventListener('DOMContentLoaded', () => {
                initializeLeafletMap('mapRumah', parseFloat({{ $pelanggan->rumah_latitude }}), parseFloat(
                    {{ $pelanggan->rumah_longitude }}), 'Rumah Tersimpan');
